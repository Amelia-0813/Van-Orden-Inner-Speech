...
// NOTE: the hard response deadline has been removed for now. After the mask,
// a "?" is shown at the center of the screen to indicate the response
// window is open, and the trial simply waits until the participant responds
// (no trial_duration / timeout).
const FEEDBACK_DURATION_MS      = 1000;  // how long practice feedback is shown
const POST_RESPONSE_DELAY_MS    = 300;   // blank delay after the response (the "?" disappears) before the next-trial fixation cross appears
const NEXT_TRIAL_FIXATION_MS    = 500;   // how long the fixation cross marking the start of the next trial is shown -- currently matched to ISI_DURATION_MS, adjust as needed
...
function buildTrialSequence(trial, jsPsych, isPractice) {
  ...
  // Pattern masking + response window: the word is visible for
  // wordDurationForThisTrial ms, then immediately replaced (same position,
  // same trial) by a string of "X" the same length as the word. The mask
  // stays on screen for MASK_DURATION_MS ms, then is replaced by a "?" at
  // the center of the screen to indicate the response window is open.
  // There is currently no response deadline: the trial simply waits for the
  // participant to press a key (no trial_duration set below).
  ...
  const wordScreen = {
    ...
    // No trial_duration -- the response deadline is removed for now, so
    // this trial waits indefinitely until the participant responds.
    data: {
      screen: "word",
  ...
  // Once the participant responds, the "?" disappears (the word screen
  // simply ends). After a POST_RESPONSE_DELAY_MS blank, a fixation cross is
  // shown to mark the beginning of the next trial.
  sequence.push({
    type: jsPsychHtmlKeyboardResponse,
