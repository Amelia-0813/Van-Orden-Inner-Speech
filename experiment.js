...
const WORD_DURATION_MS          = 600;   // how long the real word is visible before being masked (main trials)
const PRACTICE_WORD_DURATION_MS = 600;   // how long the real word is visible before being masked (practice trials)
...
async function runExperiment() {
  ...
  const EXAMPLE_BOX_STYLE = `
    <style>
      ...
      .example-trial-box .example-arrow {
        font-size: 24px;
        color: #888;
      }
    </style>
  ...
  timeline.push({
    ...
    pages: [
      `<div class="instructions-block">
         ...
         <p>In this experiment, you will first see a category and then a word. Your task is to decide if the word belongs to a category or not.</p>
      ...
      `<div class="instructions-block">
         ...
         <p>You should press <strong>"${KEY_YES}"</strong> to indicate "yes".</p>
       </div>`,
      ...
      `<div class="instructions-block">
         ...
         <p>You should press <strong>"${KEY_NO}"</strong> to indicate "no".</p>
       </div>`,
    ],
  ...
  // ---- Final reminder page: requires pressing "q" to proceed ----
  // Pulled out of jsPsychInstructions above because that plugin's
  // key_forward/key_backward apply to the whole page set -- there's no way
  // to give just one page inside it a different advance key. As a standalone
  // trial, it's simple to restrict `choices` to just "q".
  timeline.push({
    type: jsPsychHtmlKeyboardResponse,
    stimulus: `<div class="instructions-block">
        <p><strong>Respond as accurately and quickly as possible!</strong></p>
        <p><strong>There is a limited time to respond. So make sure you click your answer quickly.</strong></p>
        <p><strong>If you do not respond within the time window, your answer will be counted as incorrect.</strong></p>
        <p>You will start with some practice trials. Then you will move into the experiment. Finally, you will complete a questionnaire.</p>
  ...
  timeline.push({
    ...
    stimulus: `<div class="instructions-block">
        ...
        <p><strong>Remember: press "${KEY_YES}" for yes, "${KEY_NO}" for no.</strong> Respond as quickly and accurately as possible.</p>
